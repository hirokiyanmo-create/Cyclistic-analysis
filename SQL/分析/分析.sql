...
SELECT
  member_casual, start_station_name, ride_count, rnk
FROM RANKING
WHERE rnk <= 10



--会員別/開始駅としての利用数と終了駅としての利用数比較(開始駅利用駅基準)--

WITH start as(
SELECT
  member_casual,
  start_station_name,
  COUNT(*) As st_ride_num
FROM
  `cyclistic-analysis-506602.cyclistic_analysis.trips_clean` 
WHERE start_station_name IS NOT NULL
GROUP BY member_casual,start_station_name
ORDER BY st_ride_num DESC
),
...
SELECT
  member_casual,
  end_station_name,
  COUNT(*) As end_ride_num
FROM
  `cyclistic-analysis-506602.cyclistic_analysis.trips_clean` 
WHERE end_station_name IS NOT NULL
GROUP BY member_casual,end_station_name
ORDER BY end_ride_num DESC)

SELECT
  COALESCE(st.member_casual, END_st.member_casual) AS member_casual,
  COALESCE(st.start_station_name, END_st.end_station_name) AS station_name,
  st.st_ride_num,
  END_st.end_ride_num  
FROM start AS st
FULL OUTER JOIN END_st  
  ON st.start_station_name = END_st.end_station_name
  AND st.member_casual = END_st.member_casual
ORDER BY st.st_ride_num DESC
